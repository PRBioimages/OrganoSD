...
def json():
    ...
    json_folder = "/home/hryang/Detecseg/tracking/searching/metric/annotations.json"
    mask_folder = "/home/hryang/Detecseg/tracking/searching/metric/mask"
    box_folder = "/home/hryang/Detecseg/tracking/searching/metric/box"
    img_foder  ='/home/hryang/Detecseg/tracking/searching/metric/img/'
    ...
    for imgname, anno in data.items():
        ...
        for shape in anno:
            if shape['type'] == 'polygon':
                ...
                # patch = img[max(0,xmin-20):min(xmax+20, img_width), max(0,ymin-20):min(ymax+20, img_height)]
                patch = img[max(0, ymin - 20):min(ymax + 20, img_height), max(0, xmin - 20):min(xmax + 20, img_width)]
                mask = cv2.fillPoly(mask, [points], color=255)
                # patch_mask = mask[max(0,xmin-20):min(xmax+20, img_width), max(0,ymin-20):min(ymax+20, img_height)]
                patch_mask = mask[max(0, ymin - 20):min(ymax + 20, img_height), max(0, xmin - 20):min(xmax + 20, img_width)]
                j += 1
            ...
            if shape['type'] == 'ellipse':
                ...
                mask = cv2.ellipse(mask, (int(cx), int(cy)), (int(rx), int(ry)), angle, 0, 360, 255, -1)
                # patch = img[max(0,xmin-20):min(xmax+20, img_width), max(0,ymin-20):min(ymax+20, img_height)]
                patch = img[max(0, ymin - 20): min(ymax + 20, img_height), max(0, xmin - 20): min(xmax + 20, img_width)]
                # patch_mask = mask[max(0,xmin-20):min(xmax+20, img_width), max(0,ymin-20):min(ymax+20, img_height)]
                patch_mask = mask[max(0, ymin - 20): min(ymax + 20, img_height), max(0, xmin - 20): min(xmax + 20, img_width)]
                j += 1
            imgname = os.path.splitext(imgname)[0] + str(j) + '.jpg'
            cv2.imencode('.jpg', patch_mask)[1].tofile(os.path.join(mask_folder, imgname))
            cv2.imencode('.jpg', patch)[1].tofile(os.path.join(box_folder, imgname))
