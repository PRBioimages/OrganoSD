# import locoefa.computecoeff
# import locoefa.initialize
# import locoefa.reconstruct
...
from locoefa import initialize
from locoefa import reconstruct
...
def compute_LOCOEFA_Lcoeff(mask, down_shape=1., N_modes=50, DEBUG=False):
    ma_down = transform.resize(mask.astype(float), (int(mask.shape[0] * down_shape), int(mask.shape[1] * down_shape)),
...
if __name__ == '__main__':
    ...
    for i in range(len(file_name_list)):
        ...
        contour, mode = initialize.read_example_data(fname)
        ...
        for mm in tqdm.trange(max_mode, desc='Max_modes'):
            x_loco, y_loco = reconstruct.reconstruct_contour(mode, tp=tp, rec_type='LOCOEFA', first_mode=0,
                                                             last_mode=mm)
            x_efa, y_efa = reconstruct.reconstruct_contour(mode, tp=tp, rec_type='EFA', first_mode=0, last_mode=mm)
