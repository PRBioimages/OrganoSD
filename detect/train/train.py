...
from model.OrgnoSD import ODSeg_U
from data.dataset import DetecSegSet, ToPercentCoords
from data.augment import ODSegAugment
from loss.multibox_loss import DetecLoss, SegLoss_focal, total_loss
...
parser.add_argument('--train_set_root', default='/WPC/one_image',
                    help='Dataset root directory path')
parser.add_argument('--val_set_root', default='/WPC/one_image',
                    help='Dataset root directory path')
parser.add_argument('--save_folder', default='/params/241024/',
                    help='Directory for saving checkpoint models')
parser.add_argument('--save_img', default='/train_img/241024/',
                    help='Directory for saving checkpoint models')
parser.add_argument('--resume', default=False,    #'/params/240319/21.pth'
...
def train():
    ...
    net = ODSeg_U(phase='train', in_channels=1, unet_out_channels=1, num_classes=num_classes)
    ...
    while epoch < 200:
        ...
        for i, (img, box, mask) in enumerate(tqdm.tqdm(data_loader_train)):
            ...
            train_loss = loss_s + loss_c + loss_l
            opt.zero_grad()
            train_loss.backward()
            opt.step()
            ...
            if i % 20 == 0:
                ...
                image = torch.cat((img_box, _out_detec, _out_seg_, _mask),dim=1).detach().numpy()
