...
parser.add_argument('--test_setroot', default='/WPC/val/',
                    help='Dataset root directory path')
parser.add_argument('--resume', default='/params/240408/49.pth',
                    type=str, help='Checkpoint state_dict file to resume training from')
parser.add_argument('--save_img', default="/test_img/20240325/",
...
# net = ODSeg_U('train', 1, 1, cfg1013, num_classes)
# net = net.to(device)
#
# if args.resume:
...
# else:
#     print('Please initialize weights...')
#
...
#     out_detect, out_seg = net(Variable(img.float(), requires_grad=False))
#     out = out_seg[0].squeeze(0) * 255
#     out = out.detach().numpy().astype(np.uint16)
...
def test_net(save_folder, net, testset, device, thresh):
    ...
    for i in range(num_images):
        ...
        img = Image.open(os.path.join(args.test_setroot, 'img/' + imgname))  # 以RGB的形式读入数据
        img = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)  # 将PIL.Image转换成OpenCV格式，便于后续使用.rectangle函数
        colorg = (0, 255, 0)
        colorp = (255, 0, 0)
