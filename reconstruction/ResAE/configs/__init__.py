...
#对象的属性转换为字符串，然后使用逗号和空格分隔属性名称和属性值，最终返回一个以逗号和空格分隔的字符串，其中包括了对象的所有属性及其对应的数值。
class Element:
    def __repr__(self):
...
class Experiment(Element):
    def __init__(self, dict):
        ...
        self.Filter = dict.get('Filter', False)
        self.radom_sample = dict.get('radom_sample', False)
...
class Data(Element):
    def __init__(self, dict):
        self.data_dir = dict.get('data_dir', 'none')
        self.df_dir = dict.get('df_dir', 'none')
        self.train_img_type = dict.get('train_img_type', 'cell')
        self.img_dir = dict.get('img_dir', 'none')


class Model(Element):
    def __init__(self, dict):
        self.name = dict.get('name', 'AutoEncoder')
...
def get_config(name):
    ...
    #return load(os.path.join('/home/xlzhu/heying/CTCs/ResAE/configs', name))

    return load(os.path.dirname(os.path.realpath(__file__)) + '/' + name)


if __name__ == '__main__':
    print ('hi')
    args = get_config('config.yaml')
    #args = get_config('D:\pythonproject\S2ConvolutionAutoEncoder\configs')
