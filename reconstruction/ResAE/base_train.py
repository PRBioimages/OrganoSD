...
# 要的训练函数，负责执行模型的训练和验证
def base_train(cfg: Config, model, train_dl, valid_dl, loss_func, optimizer, save_path, scheduler, writer, tune=None):
    ...
    try:
        ...
        for epoch in range(cfg.train.num_epochs):
            ...
            for i, (image_batch) in enumerate(tq):
                # image_batch = image_batch.transpose(0, 1)
                ...
                decoded_data = model(Variable(image_batch, requires_grad=True))
            ...
            if i % 100 == 0:
                ...
                print(save_path / 'checkpoints/f{}_epoch-{}.pth'.format(cfg.experiment.run_fold, epoch))
            ## 验证模型，并计算验证损失。此外，生成图像对比以供分析
            fig_path = os.path.join(save_path, 'figure')
            # validate_loss, val_losses_class, val_losses_rec = basic_validate(model, valid_dl, loss_func, cfg,
            #                                                                  os.path.join(fig_path, 'plotoutput'),
            #                                                                  writer, epoch, device)
            validate_loss, val_losses_class, val_losses_rec = 0, 0, 0
            ...
            writer.add_scalar('train_f{}/loss_class'.format(cfg.experiment.run_fold), np.mean(train_losses_class),
                              epoch)
...
def basic_validate(mdl, dl, loss_func, cfg, save_path, writer, epoch, device):
    # weight_class = 0
    # if epoch >= 20:
    #     weight_class = 5
    # elif epoch >= 50:
    ...
    with torch.no_grad():
        ...
        for i, (ipt) in enumerate(dl):
            ipt = ipt.float().to(device)
