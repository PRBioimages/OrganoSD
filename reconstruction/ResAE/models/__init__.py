...
from reconstruction.ResAE.models.AE_residual import ResAE
from reconstruction.ResAE.configs import Config
import torch

#据配置参数 cfg 中的模型名称（cfg.model.name）来选择初始化不同的深度学习模型
def get_model(cfg: Config):
    if cfg.model.name in ['AE']:
        print(f'[ ! ] Init a AE.')
        return AE(encoded_space_dim=cfg.model.param.get('encoded_space_dim', 0))
    elif cfg.model.name in ['AEadd']:
        print(f'[ ! ] Init a AEadd.')
        return ResAE(encoded_space_dim=cfg.model.param.get('encoded_space_dim', 0))
