...
class AE_residual(nn.Module):
    def __init__(self, encoded_space_dim=512, num_classes=2):
        super().__init__()
        n_latent_dim = encoded_space_dim
        num_class = num_classes
        self.encoder = Enc(n_latent_dim=n_latent_dim)
        self.decoder = Dec(n_latent_dim=n_latent_dim)
        self.class_fc = nn.Linear(n_latent_dim, num_class)

        self.class_out = None

    def forward(self, x):
        latent_code = self.encoder(x)
        class_out = self.class_fc(latent_code)
        self.class_out = class_out.detach()
        #[2,512]
